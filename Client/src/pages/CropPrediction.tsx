...
export default function CropPrediction() {
  ...
  const getFertilizerAdvice = async () => {
    ...
    try {
      ...
      const prompt = `You are an expert agricultural advisor. A farmer needs detailed fertilizer advice for growing ${selectedCrop}.
      ...
      - pH Level: ${soilParams.ph}
      - Rainfall: ${soilParams.rainfall} mm
      - Temperature: ${soilParams.temperature} °C
      - Humidity: ${soilParams.humidity} %

      Based on these parameters and the specific requirements for ${selectedCrop}, provide a comprehensive fertilizer recommendation. Include:
      1. Specific fertilizer types (e.g., Urea, DAP, MOP) and quantities.
      2. Timing of application (basal, top dressing).
      3. Any micronutrient corrections if likely needed based on pH.
      4. Organic alternatives if applicable.
      
      Keep the advice practical, easy to understand, and safe for the crop. Format the output in clear paragraphs or bullet points within a plain text response (no markdown).`;
      ...
      const advice = data.choices[0]?.message?.content || "Could not generate advice. Please try again.";

      setFertilizer(advice);
    } catch (error) {
