...
const Market: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialPrices = async () => {
      ...
      try {
        ...
        setRecords(recs);
        setFiltered(recs);
        const uniqueStates = Array.from(new Set(recs.map((r: MandiRecord) => r.state))).sort();
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-1">Market Overview</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        />
        <StatCard title="Active States" value={stats.activeMarkets.toString()} change="Live" icon={BarChart3} trend="neutral" />
        <StatCard title="Trading Volume" value="₹45.2L" change="Today" icon={DollarSign} trend="neutral" />
        <StatCard title="Top Performer" value={stats.topPerformer} change="Best" icon={TrendingUp} trend="up" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-3 sm:mb-0">Current Market Prices</h2>
            <select
              value={selectedState}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col h-[36rem]">
          <h2 className="text-xl font-semibold text-gray-800 mb-6 shrink-0">Today's Market News</h2>
          <div className="flex-grow overflow-y-auto pr-2">
            {newsLoading ? (
              <div className="flex justify-center items-center h-full"><Loader2 className="h-8 w-8 animate-spin text-green-600" /></div>
            ) : news.length === 0 ? (
              <p className="text-gray-500 text-sm text-center">No news available right now.</p>
            ) : (
          ...
          >
            {moreNewsLoading ? (<Loader2 className="h-5 w-5 animate-spin" />) : ('More News +')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`text-sm font-medium px-2 py-1 rounded-full ${trendClasses[trend]}`}>{change}</span>
      </div>
